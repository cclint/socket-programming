...
def part4(s2: socket) -> socket:
    ...
    s_3.bind(
        (HOST, ServerPorteeeee)
    )  # binds the port number eeeee to the server's socket
    modifiedMessage, serverAddress = s_3.recvfrom(2048)
    print(
        "Recieved packets from s_3 port eeeee:" + modifiedMessage.decode()
    )  # prints the string recieved from robot on UDP connection s_3
    print("")
...
def main(s1,s2, s3): # entry point of the program and closes each socket at the end
    socket_1 = part2()
    socket_2 = part3(socket_1)
    socket_3, message, ServerPortfffff = part4(socket_2)
    p5 = part5(socket_3, message, ServerPortfffff)
    s1.close()
    s2.close()
    s3.close()
    return p5
